...
1. **Cost management.** Each Claude Code invocation uses API credits. Should the bot surface token usage per session? Should there be per-user daily limits? **Decision: surface cost in Phase 1, enforce limits in Phase 3.** Parse the final lines of Claude Code's stdout for cost/token data and append it as a footer to the session completion message. Per-user daily dollar limits (`maxDailyCostPerUser`) are deferred to Phase 3 when persistent storage is being built anyway.
1. **Persistent sessions.** If the bot restarts, all sessions are lost. Options: persist session metadata to disk, or accept that sessions are ephemeral and users re-create threads as needed. **Decision: ephemeral sessions with restart notifications.** Do not persist PTY or session state. Write active thread IDs to `~/.discord-claude/active-threads.json`; on restart, post a notification in each listed thread and clear the file.
